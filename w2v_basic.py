...
batch, labels = generate_batch(batch_size=8, num_skips=8, skip_window=4)
...
# Number of negative examples to sample per batch (about half of batch size)
num_sampled = 64
...
def device_for_node(n):
  ...
  if n.type == "MatMul" or n.type == "Sum" or n.type == "Add" or n.type == "reduce_sum":
    ...
    print "n.type = ",n.type,"device count = ",device_for_node.cpu_device_count
...
with graph.as_default():
    ...
    with graph.device(device_for_node):
        ...
        embed = tf.reshape(tf.nn.embedding_lookup(embeddings, train_input_small),[batch_size/num_skips,embedding_size])
        ...
        for i in range(num_skips):
            nce_weights_array.append(tf.Variable(tf.truncated_normal([vocabulary_size, embedding_size],stddev=1.0 / math.sqrt(embedding_size))))
        for i in range(num_skips):
            nce_biases_array.append(tf.Variable(tf.zeros([vocabulary_size])))
        print("nce_weights_array",nce_weights_array[i])
        print("nce_biases_array",nce_biases_array[i])
        print("embed",embed)
        print("train_labels_array",train_labels_array[i])
        ...
        for i in range(num_skips):
            ...
            if(loss_sum == None):
                ...
                loss_sum = tf.add(loss_sum, nce_loss_array[i])
